...
# Limitar precisão dos dados. Obs.: Só afeta os prints relacionados ao Numpy.
...
def main():
    ...
    if met != "d":
        ...
        if m != int(
            m
        ):  # Arredondamento do parâmetro "m" segue convenção explicada em aula.
            decimal = m - int(m)
            arredondado = int(m)
...
def plot(v_aprox, v_exato, teste):
    ...
    if teste != "c":
        ...
        plt.scatter(
            position, np.abs(np.array(v_aprox) - np.array(v_exato)), marker="^", c="k"
        )
...
def vetor_exato(vetor, m, t, k, teste):
    """Recebe os parâmetros "vetor", "m", "t", "k" e "teste" retornando
...
def fonte(x, t, teste):
    """Recebe os parâmetros "x", "t" e "teste" retornando um número real
    correspondente à expressão determinada pelo teste que o usuário deseja rodar."""
    ...
    if teste == "a":
        ...
        return 10 * math.cos(10 * t) * (x**2) * (1 - x) ** 2 - (
            1 + math.sin(10 * t)
        ) * (12 * (x**2) - 12 * x + 2)
...
def u(vetor, m, t, lda, met, teste):
    ...
    uold = (
        vetor.copy()
    )  # Recebe o parâmetro "vetor" que contém a condição de contorno u0 (primeira linha)
    ...
    if met == "d":  # Método da discretização (explícito)
        ...
        for k in range(m):
            i = 1
            while i < n:
                unew[i] = uold[i] + delta_t * (
                    (uold[i - 1] - 2 * uold[i] + uold[i + 1]) / delta_x**2
        ...
        for k in range(m):
            for d in range(
                1, n
            ):  # Preenchimento da matriz coluna b para o método de Euler
                if d == 1:
                    vet_dir[d] = (
                        uold[d]
                        + (delta_t * fonte(d * delta_x, (k + 1) * delta_t, teste))
            ...
            unew[0] = g1(
                (k + 1) * delta_t, teste
            )  # Preenchimento da primeira e última posição.
            unew[n] = g2((k + 1) * delta_t, teste)
            uold = np.array(unew.copy())
            ...
            for d in range(
                ...
                if d == 1:
                    vet_dir[d] = (
                        ...
                        + (delta_t / 2)
                        * (
                            fonte(d * delta_x, (k + 1) * delta_t, teste)
                            + fonte(d * delta_x, k * delta_t, teste)
                        )
...
def crie_vetor(n_colunas):
    """Recebe o parâmetro 'n_colunas' e retorna um vetor nulo de dimensão n+1."""
